...
# https://github.com/gohugoio/hugo/releases/download/v0.154.2/hugo_extended_0.154.2_linux-amd64.tar.gz
# mv hugo /usr/bin/hugo-0.154.2-ex

VER=`grep "^HUGO_VERSION" $srcdir/netlify.toml | \
  sed -e "s/HUGO_VERSION = \"\([0-9\.]*\)\"/\1/"`
echo ***MATSUAND***
echo hugo binary should be $VER
echo hugo binary is hugo-0.154.2-ex from ./2.make.sh
echo ***MATSUAND***
cd src
hugo-0.154.2-ex --gc --minify -b http://lfstranslation5/docker.docs-ja #--environment development

#npx pagefind@v1.3.0 --force-language ja
